...
import { watchLabel } from '../watch-label';

// Re-exported from their original home so existing imports keep working;
// new code should import from $lib/server/errors and $lib/watch-label.
export { StateError, watchLabel };
...
export async function assertWatchOwned(db: DB, userId: number, watchId: number) {
	const row = (
		await db
...
export async function lockUser(tx: DB, userId: number) {
